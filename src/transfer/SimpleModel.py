...
class SimpleModel(object):
    ...
    def __init__(self,
        ...
        with vs.variable_scope("basic_rnn_seq2seq"):
            outputs, self.states = tf.contrib.rnn.static_rnn(cell, [x_p], dtype=tf.float32)

        self.outputs = outputs

        with tf.name_scope("loss_angles"):
            loss_angles = tf.reduce_mean(tf.sqrt(tf.square(tf.subtract(y_p, outputs[0][-1]))))
            # loss_angles = tf.sqrt(tf.square(tf.subtract(y_p[0], outputs[0][-1])))
