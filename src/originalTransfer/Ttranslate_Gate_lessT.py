...
tf.app.flags.DEFINE_integer("learning_rate_step", int(10000), "Every this many steps, do decay.")
tf.app.flags.DEFINE_float("max_gradient_norm", 5, "Clip gradients to this norm.")
tf.app.flags.DEFINE_integer("batch_size", 16, "Batch size to use during training.")
tf.app.flags.DEFINE_integer("iterations", int(10000), "Iterations to train for.")
# Architecture
...
tf.app.flags.DEFINE_string("train_dir", os.path.normpath("./experiments_TL_SourceOnly/"), "Training directory.")
...
tf.app.flags.DEFINE_integer("transfer", 1, "If transfer from existing models")
tf.app.flags.DEFINE_integer("transfer_point", int(1e4), "transfer model")
tf.app.flags.DEFINE_integer("transfer_iteration", int(1e4), "transfer model")
tf.app.flags.DEFINE_float("transfer_rate", .05, "Learning rate.")
tf.app.flags.DEFINE_string("transfer_folder", "experiments_sufficient_lr0.5", "transfer folder.")
tf.app.flags.DEFINE_integer("TL_seq_length_out", 10, "Number of frames that the decoder has to predict. 25fps")
# tf.app.flags.DEFINE_string("transfer_actions","all","transfer source actions")
# tf.app.flags.DEFINE_string("transfer_actions",["eating"],"transfer source actions")
tf.app.flags.DEFINE_string("transfer_actions", ["walking", "eating", "smoking", "discussion", "directions",
                                                "greeting", "phoning", "posing", "purchases", "sitting",
                                                "sittingdown", "takingphoto", "waiting", "walkingdog",
                                                "walkingtogether"], "transfer source actions")

FLAGS = tf.app.flags.FLAGS
FLAGS.transfer_actions.remove(FLAGS.action)
print(FLAGS.transfer_actions)
...
# wirte log files --by xxr------
fname = os.getcwd() + "/Results_TL_stack_source_only/"
...
d = fname + "/{0}_TL_sourceOnly.log".format(FLAGS.action)
...
sys.stdout = log_file
# print("MA test:")
# wirte log files --by xxr----<end>-----

summaries_dir = os.path.normpath(os.path.join(train_dir, "log"))  # Directory for TB summaries
print('summaries dir:', summaries_dir)


def create_model(session, actions, sampling=False):
    """Create translation model and initialize or load parameters in session."""
    # transfer: load source models:----------------------------------------------

    if FLAGS.transfer > 0:  # do transfer learning
        print("create model with transfer-----------")
        # source_model_list=[]

        # for action in FLAGS.transfer_actions:

        model_source = seq2seq_model.Seq2SeqModel(
            actions,
            FLAGS.architecture,
        ...
        model_target = Tseq2seq_model_Gate.TSeq2SeqModel(
            session,
            FLAGS,
            model_source,
            FLAGS.architecture,
    ...
    # - end transfer model loading...----------------------------------------------------------

    # no tranfer:
    else:
        print("create model without transfer-----------")
            ...
            if FLAGS.load > 0:
                if os.path.isfile(os.path.join(train_dir, "checkpoint-{0}.index".format(FLAGS.load))):
                    ckpt_name = os.path.normpath(
                        os.path.join(os.path.join(train_dir, "checkpoint-{0}".format(FLAGS.load))))
                else:
...
def train():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, device_count=device_count)) as sess:
        ...
        # === Read and denormalize the ground truth with srnn's seeds, as we'll need them
        ...
        for _ in xrange(FLAGS.iterations):
            ...
            if current_step % FLAGS.test_every == 0:
                ...
                for action in actions:
                    ...
                    print("begin to test action: ", action)
                    ...
                    for i in np.arange(N_SEQUENCE_TEST):
                        ...
                        euc_error = np.power(
                            srnn_gts_euler[action][i][:, idx_to_use] - eulerchannels_pred[:, idx_to_use], 2)
                        euc_error = np.sum(euc_error, 1)
                        euc_error = np.sqrt(euc_error)
                        mean_errors[i, :] = euc_error

                    print("begin calculate mean_mean_error:")

                    # This is simply the mean error over the N_SEQUENCE_TEST examples
                    mean_mean_errors = np.mean(mean_errors, 0)
                    print("finish calculate mean_mean_error:")

                    print("begin print results for 80,160,...")
...
def sample():
    ...
    device_count = {"GPU": 0} if FLAGS.use_cpu else {"GPU": 1}
    with tf.Session(config=tf.ConfigProto(device_count=device_count)) as sess:  # using GPU or not
        ...
        # === Read and denormalize the groundtruth with srnn's seeds, as we'll need them
        # many times for evaluation in Euler Angles ===
