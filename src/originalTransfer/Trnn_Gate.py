...
def static_rnn(session,
  ...
  # determined by the parent scope, or is set to place the cached
  # Variable using the same placement as for the rest of the RNN.
  with vs.variable_scope(scope or "Trnn") as varscope:
    if varscope.caching_device is None:
      varscope.set_caching_device(lambda op: op.device)
    ...
    if sequence_length is not None:  # Prepare variables
      ...
      def _create_zero_output(output_size):
        ...
        output = array_ops.zeros(
            array_ops.stack(size), _infer_state_dtype(dtype, Tstate))
    ...
    for time, input_ in enumerate(inputs):
        ...
        (Toutput, Tstate) = cal_out_state(sequence_length, time, min_sequence_length, max_sequence_length, zero_output, Tstate, call_cell,
                    cell)

       # transfer: source rnn's state and output--------------------------------
        state_list=[]
        output_list=[]
        e_h=[]
        e_s=[]
        ...
        for action in FLAGS.transfer_actions:
            print("model with action:",action)
            transfer_dir = os.path.normpath(os.path.join(pa, FLAGS.transfer_folder,action,
                                                         'out_{0}'.format(FLAGS.TL_seq_length_out),
                                                         'iterations_{0}'.format(FLAGS.transfer_iteration),
                                                         FLAGS.architecture,
                                                         FLAGS.loss_to_use,
                                                         'omit_one_hot' if FLAGS.omit_one_hot else 'one_hot',
                                                         'depth_{0}'.format(FLAGS.num_layers),
                                                         'size_{0}'.format(FLAGS.size),
                                                         'lr_{0}'.format(FLAGS.transfer_rate),
                                                         'residual_vel' if FLAGS.residual_velocities else 'not_residual_vel'))

            # print("transfer_dir", transfer_dir)
            model_path=os.path.normpath(os.path.join(transfer_dir,"checkpoint-{0}".format(FLAGS.transfer_point)))

            state_list, output_list = TL_get_source_states(model_path, action, model_source, session, input_, Tstate, sequence_length,
                                     time, min_sequence_length, max_sequence_length, zero_output,
                                     state_list, output_list)
            k=k+1

        output, Tstate = TL_get_final_output(w_h,w_s,w_t_h, Toutput, w_t_s, Tstate, state_list, output_list) # get final states based on the source states list.
        outputs.append(output)

    return (outputs, Tstate)

def TL_get_source_states(model_path,action,model_source,session,input_,state,sequence_length,time,min_sequence_length, max_sequence_length,zero_output,state_list,output_list):

    print("Loading transfer model {0} from action {1}".format(model_path, action))
    ...
    (Soutput, Sstate) = cal_out_state(sequence_length, time, min_sequence_length, max_sequence_length,
                                      zero_output, state, call_cell_source, model_source)

    # multi = tf.matmul(w_h, Soutput)
    # e_h_1 = tf.tanh(multi)
    # e_h_t = tf.matmul(e_h_1, v_h)
    # print("shape of e_h_t")
    # print(e_h_t)

    # multi1 = tf.matmul(w_s, Sstate)
    # e_s_1 = tf.tanh(multi1)
    # e_s_t = tf.matmul(e_s_1, v_s)
...
# copy from tf.contrib.legacy_seq2seq.tied_rnn_seq2seq-------
...
# copied form tf.contrib.legacy_seq2seq.tied_rnn_seq2seq------
def tied_rnn_seq2seq(session,FLAGS,model_source,
                       w_h,
                       w_s,
                       w_t_h,
                       w_t_s,
  ...
  Returns:
    A tuple of the form (outputs, state), where:
      ...
      state: The state of each decoder cell in each time-step. This is a list
        ...
        It is a 2D Tensor of shape [batch_size x cell.state_size].
  """
  with vs.variable_scope("TL_combined_tied_rnn_seq2seq"):
    scope = scope or "TL_tied_rnn_seq2seq"
    _, enc_state = static_rnn(session,FLAGS,model_source,w_h, w_s,w_t_h,w_t_s,cell, encoder_inputs, dtype=dtype, scope=scope)
    vs.get_variable_scope().reuse_variables()
    return rnn_decoder(
        decoder_inputs,
        enc_state,
...
def cal_out_state(sequence_length,time,min_sequence_length,max_sequence_length,zero_output,state,call_cell,cell):
    if sequence_length is not None:
        (Toutput, Tstate) = _rnn_step(
            time=time,
            sequence_length=sequence_length,
            min_sequence_length=min_sequence_length,
            max_sequence_length=max_sequence_length,
        ...
        (Toutput, Tstate) = call_cell()

    return Toutput,Tstate
