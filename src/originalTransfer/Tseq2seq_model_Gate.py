...
class TSeq2SeqModel(object):
    ...
    def __init__(self,
        ...
        with tf.name_scope("TL_inputs"):
            ...
            if FLAGS.transfer_actions == "all":
                w_h = vs.get_variable("TL_w_h_source_{0}".format(FLAGS.transfer_actions), shape=[1, self.batch_size])
                # v_h = vs.get_variable("TL_v_h_source_{0}".format(FLAGS.transfer_actions), shape=[self.input_size, 1])
                w_s = vs.get_variable("TL_w_s_source_{0}".format(FLAGS.transfer_actions), shape=[1, self.batch_size])
                ...
                for action in FLAGS.transfer_actions:
                    print("generate weights for source_action", action)
                    w_h = vs.get_variable("TL_w_h_source_{0}".format(action), shape=[1, self.batch_size])
                    # v_h = vs.get_variable("TL_v_h_source_{0}".format(action), shape=[self.input_size, 1])
                    w_s = vs.get_variable("TL_w_s_source_{0}".format(action), shape=[1, FLAGS.size])
                    # v_s = vs.get_variable("TL_v_s_source_{0}".format(action), shape=[self.rnn_size, 1])
            ...
            self.w_t_s = vs.get_variable("TL_w_s_target", shape=[1, FLAGS.size])
            ...
            enc_in = tf.split(enc_in, source_seq_len - 1,
                              axis=0)  # split enc_in into source_seq_len-1 pieces(enc_in is a list with length (source_seq_len-1)), along axis 0.
            dec_in = tf.split(dec_in, target_seq_len, axis=0)
            dec_out = tf.split(dec_out, target_seq_len, axis=0)

        # === Add space decoder ===
        cell = Trnn_cell_extensions.LinearSpaceDecoderWrapper(cell,
                                                              self.input_size)  # linear warapper after GRU, make the output of GRU has the same dimension as input for residual connection.

        # Finally, wrap everything in a residual layer if we want to model velocities
        if residual_velocities:
            cell = Trnn_cell_extensions.ResidualWrapper(cell)
        ...
        if architecture == "basic":
            ...
            with vs.variable_scope("basic_rnn_seq2seq"):
                ...
                # outputs, self.states = tf.contrib.legacy_seq2seq.Trnn_decoder( dec_in, enc_state, cell, loop_function=lf ) # Decoder
                _, enc_state = Trnn_Gate.static_rnn(session, FLAGS, model_source, self.w_h, self.v_h, self.w_s,
                                                    self.v_s, self.w_t_h, self.v_t_h, self.w_t_s, self.v_t_s, cell,
                                                    enc_in, dtype=tf.float32)  # Encoder
                outputs, self.states = Trnn_Gate.rnn_decoder(dec_in, enc_state, cell, loop_function=lf)  # Decoder

        elif architecture == "tied":
            # outputs, self.states = tf.contrib.legacy_seq2seq.tied_rnn_seq2seq( enc_in, dec_in, cell, loop_function=lf )
            outputs, self.states = Trnn_Gate.tied_rnn_seq2seq(session, FLAGS, model_source, self.w_h, self.w_s,
                                                              self.w_t_h, self.w_t_s, enc_in, dec_in, cell,
                                                              loop_function=lf)
        else:
        ...
        params = []
        # only select target parameters to train:
        for m in params1:
            if 'TL' in m.name:
                params.append(m)

        # print trainable variable's name:---------------------------------------
        # variables_names = [v.name for v in tf.trainable_variables()]
        # values = sess.run(variables_names)
        for k in params:
            print("Trainable_Variables: ", k.name)
    ...
    #     _, enc_state = Trnn.static_rnn(source_model_list,
    #       cell, encoder_inputs, dtype=dtype, scope=scope)
    #     variable_scope.get_variable_scope().reuse_variables()
    #     return self.Trnn_decoder(
    #       decoder_inputs,
    #       enc_state,
    #       cell,
    ...
    def find_indices_srnn(self, data, action):
        ...
        idx = []
        for i in range(np.int32(self.batch_size / 2)):
    ...
    def get_batch_srnn(self, data, action):
        ...
        # print("test: length of frames:",frames)
