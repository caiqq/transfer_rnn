...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    with tf.name_scope("inputs_"):
      ...
      enc_in = tf.split(enc_in, source_seq_len-1, axis=0) # split enc_in into source_seq_len-1 pieces(enc_in is a list with length (source_seq_len-1)), along axis 0.
      dec_in = tf.split(dec_in, target_seq_len, axis=0)
      dec_out = tf.split(dec_out, target_seq_len, axis=0)

    # === Add space decoder ===
    cell = rnn_cell_extensions.LinearSpaceDecoderWrapper( action,cell, self.input_size ) # linear warapper after GRU, make the output of GRU has the same dimension as input for residual connedction
