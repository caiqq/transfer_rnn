...
class LinearSpaceDecoderWrapper(RNNCell):
  ...
  def __init__(self, action, cell, output_size):
    ...
    if isinstance(self._cell.state_size,tuple):
      ...
      # with tf.variable_scope("LSDP",reuse=True):
      self.w_out = tf.get_variable("proj_w_out_",
          [insize, output_size],
          dtype=tf.float32,
          initializer=tf.random_uniform_initializer(minval=-0.04, maxval=0.04))
      # self.w_out.reuse=True
      self.b_out = tf.get_variable("proj_b_out_", [output_size],
          dtype=tf.float32,
          initializer=tf.random_uniform_initializer(minval=-0.04, maxval=0.04))
      # self.b_out.reuse=True
      self.linear_output_size = output_size

    # tf.get_variable_scope().reuse_variables()
    # self.linear_output_size.reuse=True
